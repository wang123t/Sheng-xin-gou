  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="user in userAddress" :key="user.id">
        ...
        <p @click="changeAddress(user, userAddress)">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="order in orderTradeInfo.detailArrayList"
          :key="order.id"
        >
          <li>
      ...
      <ul>
        <li>
          <b
            ...
            >件商品，总商品金额</b
          >
          <span>¥{{ orderTradeInfo.originalTotalAmount }}.00</span>
        </li>
        <li>
          <b>返现:</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:<span>¥{{ orderTradeInfo.originalTotalAmount }}.00</span>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      orderTradeInfo: (state) => state.trade.orderInfo||{},
    }),
    //最终收货地址
    userAddressInfo() {
  ...
  methods: {
    //修改收货地址
    changeAddress(user, userAddress) {
    ...
    async goPay() {
      ...
      if (result.code == 200) {
        ...
        //提交订单成功将返回的订单号通过路由传参，传给pay组件
        this.$router.push({ name: "pay",query:{orderCode:this.orderCode},params:{totalNum:this.orderTradeInfo.originalTotalAmount} });
      }else{
  ...
  .content {
    ...
    .address {
      ...
      .username::after {
        ...
        right: 0;
        bottom: 0;
        background: url(./image/choosed.png) no-repeat;
      }
